...
function inputChange(e) {
    ...
    fetchCountries(name)
        ...
        generalCountryList(countries);
        })
        .catch(err => Notify.failure('Oops, there is no country with that name'));
}

function generalCountryList(countries) {
    if (countries.length === 1) {
        generalCountryInfo(countries);
    } else {
        generalCountrysList(countries);
    }
}

function generalCountryInfo(country) {
...
function generalCountrysList(countries) {
